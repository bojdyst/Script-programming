...
function requestListener(request, response) {
	...
	if (url.pathname == '/submit') { // Processing the form content, if the relative URL is '/submit'
		...
		if (request.method == 'GET') // If the GET method was used to send data to the server
			...
			check(response, url.searchParams.get('path')); //d "url.searchParams.get('name')" contains the contents of the field (form) named 'name'
		...
		response.write(`<form method="GET" action="/submit">
	    					<label for="path">Give path</label>
	    					<input name="path">
	    					<br>
	    					<input type="submit">
	    					<input type="reset">
...
function check(response, path) {
    fs.stat(path, (err, stats) => {
        ...
        if (stats.isFile()) {
            response.write(`${path} is a file with content: `);
